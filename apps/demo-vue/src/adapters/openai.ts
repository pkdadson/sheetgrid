...
export function makeOpenAISend(opts: { apiKey: string; model: string }) {
  return async (input: SendInput): Promise<SendOutput> => {
    const { default: OpenAI } = await import("openai");
    const client = new OpenAI({
      apiKey: opts.apiKey,
      dangerouslyAllowBrowser: true, // Dev testing only.
    });
    ...
    for (const m of input.messages) {
      if (m.role === "user") {
        ...
        // tool_result → separate messages, one per result, keyed by tool_call_id
