...
import { type OpResult, ok } from "../types/op-result.js";
  ...
  return [
    {
      name: "grid_undo",
      description:
        "Reverse the last mutation. Returns not_found if the undo stack is empty.",
      input_schema: {
        type: "object",
        properties: {},
    ...
    },
    {
      name: "grid_restore",
      description:
        "Restore grid state from a snapshot taken via grid_snapshot. The restore is itself undoable.",
      input_schema: {
        type: "object",
        required: ["snapshot"],
        properties: { snapshot: {} },
      ...
      async execute(input) {
        return c.restore(
          (input as { snapshot: any }).snapshot,
        ) as OpResult<unknown>;
      },
    },
  ];
