...
export function Grid(props: GridProps) {
  ...
  const cutSelection = useCallback(async () => {
    if (!primaryRange) return;
    await copySelection();
    const matrix = extractRange(store, primaryRange.start, primaryRange.end);
  ...
  const onCellMouseDown = (
    ...
    if (editing && store.isFormulasEnabled() && isFormulaDraft(editing.draft)) {
  ...
  const onResizeMouseDown = (e: ReactMouseEvent, columnId: ColumnId) => {
    e.preventDefault();
    e.stopPropagation();
    resizeRef.current = {
      columnId,
          ...
          >
            ...
            <thead>
              {clippedHeaderLevels.map((level, levelIndex) => {
                const isLeafLevel =
                  levelIndex === clippedHeaderLevels.length - 1;
                return (
                    ...
                    {level.map((cell) => {
                      ...
                      const priority =
                        effectiveSort.length >= 2 && sortIndex >= 0
                          ...
                          : undefined;
                      return (
                        // biome-ignore lint/a11y/useKeyWithClickEvents: sort is keyboard-accessible via the grid-level keymap (Enter on a focused columnheader; see docs/keyboard-a11y.md)
                        <th
                          key={cell.id}
                          className={[
                            "eg-th",
...
function clipHeaderLevel(
  ...
  if (isLeafLevel) {
    return visibleLeafIds.map((id) => {
      const found = level.find((c) => c.id === id || c.columnIds[0] === id);
      return (
        found ?? {
          id,
          header: id,
