...
describe("view + formula + history tools", () => {
  it("grid_set_sort + grid_clear_sort", async () => {
    ...
    await t.find((x) => x.name === "grid_set_sort")!.execute({
  ...
  it("grid_set_filter", async () => {
    ...
    expect(c.getSchema().filter).toMatchObject({ column: "a", op: "gt", value: 0 });
  ...
  it("grid_set_formula + grid_clear_formula", async () => {
    const { c, store } = fx();
    const t = describeGridTools(c);
    await t.find((x) => x.name === "grid_set_formula")!.execute({
      rowId: "r1",
      columnId: "b",
      source: "=A1*2",
    });
    expect(store.getCell("r1", "b")).toBe(2);
    await t.find((x) => x.name === "grid_clear_formula")!.execute({
      rowId: "r1",
      columnId: "b",
    });
  ...
  it("grid_undo + grid_redo", async () => {
    const { c, store } = fx();
    const t = describeGridTools(c);
    await t.find((x) => x.name === "grid_set_cell")!.execute({ rowId: "r1", columnId: "a", value: 5 });
  ...
  it("grid_snapshot + grid_restore round-trip", async () => {
    ...
    const t = describeGridTools(c);
    await t.find((x) => x.name === "grid_set_cell")!.execute({ rowId: "r1", columnId: "a", value: 5 });
    const snapRes = await t.find((x) => x.name === "grid_snapshot")!.execute({});
    expect(snapRes.ok).toBe(true);
    if (!snapRes.ok) throw new Error();
    await t.find((x) => x.name === "grid_set_cell")!.execute({ rowId: "r1", columnId: "a", value: 99 });
    await t.find((x) => x.name === "grid_restore")!.execute({ snapshot: snapRes.value });
    expect(store.getCell("r1", "a")).toBe(5);
  });
});
