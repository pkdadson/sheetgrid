...
import { fail, type OpResult } from "../types/op-result.js";
...
export function createDispatcher(o: DispatcherOptions): Dispatcher {
  return (op, cmd): OpResult => {
    ...
    if (!store) return fail("detached", "GridController is not attached to a grid");
    const authRes = o.auth(op);
    if (!authRes.ok) return authRes;
    const res = store.__history.dispatch(cmd);
    if (!res.ok) return { ok: false, code: res.code as any, message: res.message, details: (res as any).details };
