...
describe("column tools", () => {
  it("add/update/delete/move column", async () => {
    ...
    await tools.find((t) => t.name === "grid_add_column")!.execute({ def: { id: "b", header: "B" } });
    expect(store.getColumns().map((x) => x.id)).toEqual(["a", "b"]);
    await tools.find((t) => t.name === "grid_update_column")!.execute({
      columnId: "b",
      patch: { header: "Bee" },
    });
    expect(store.getColumns().find((x) => x.id === "b")!.header).toBe("Bee");
    await tools.find((t) => t.name === "grid_move_column")!.execute({ columnId: "b", toIndex: 0 });
    expect(store.getColumnOrder()).toEqual(["b", "a"]);
    await tools.find((t) => t.name === "grid_delete_column")!.execute({ columnId: "a" });
